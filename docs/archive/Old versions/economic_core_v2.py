...
class ParameterRegistry:
    ...
    APPRENTICE_INITIAL_PREMIUM: Parameter = field(default_factory=lambda: Parameter(
        ...
        max_val=120000,
        tier=1,
        source="Calculated from placement data",
        unit="INR/year",
...
class CounterfactualDistribution:
    ...
    # P(Formal) for each counterfactual pathway
    ...
    def get_weighted_p_formal(self) -> float:
        """Calculate weighted average P(Formal) for control group."""
        return (
            self.p_government_school * self.p_formal_government +
            self.p_low_fee_private * self.p_formal_low_fee_private +
            self.p_dropout * self.p_formal_dropout
        )
...
class LifetimeNPVCalculator:
    ...
    LNPV = Σ_{t=0}^{T} [E[Earnings_t]^Treatment - E[Earnings_t]^Control] / (1 + δ)^t
    """
    ...
    def calculate_treatment_trajectory(
        ...
        if intervention == Intervention.RTE:
            # RTE: Higher P(Formal) due to better credentials
            p_formal = self.params.P_FORMAL_HIGHER_SECONDARY.value
    ...
    def calculate_control_trajectory(
        ...
        Uses counterfactual schooling distribution.
        """
        working_years = int(self.params.WORKING_LIFE_FORMAL.value)
        ...
        # Government school pathway
        p_formal_govt = self.counterfactual.p_formal_government
        ...
        # Low-fee private pathway (slightly better)
        p_formal_lfp = self.counterfactual.p_formal_low_fee_private
        ...
        # Dropout pathway
        p_formal_dropout = self.counterfactual.p_formal_dropout
    ...
    def calculate_npv(
        ...
        NPV = Σ_{t=0}^{T} differential_t / (1 + δ)^t
        """
