"""
RightWalk Foundation Economic Impact Model - Core Engine v3.0
====

CORRECTED VERSION incorporating PLFS 2023-24 parameters from Milestone 2.

Key Changes from v1.0:
- Mincer returns: 8.6% → 5.8% (↓32%)
- Experience premium: 4% → 0.885% (↓78%)
- Real wage growth: 2-3% → 0.01% (↓98%)
- Full 32-scenario structure (2 interventions × 4 regions × 4 demographics)
...
UPDATES in v3.0 (Scenario Framework Implementation - December 2024):
- Added Conservative/Moderate/Optimistic scenario framework (Section 10A)
- Implemented run_scenario_comparison() for systematic parameter sensitivity
- Added format_scenario_comparison() for stakeholder-friendly output
- Moderate scenario uses RWF-validated 72% apprentice placement (Nov 2025)
- Scenario framework focuses on Tier 1 critical parameters
- Integration with parameter_registry_v2_updated.py SCENARIO_CONFIGS
...
class ParameterRegistry:
    ...
    # OLD: -0.1%, NEW: -0.0123% (↑88% less negative)
    ...
    APPRENTICE_YEAR_0_OPPORTUNITY_COST: Parameter = field(default_factory=lambda: Parameter(
        ...
        max_val=-20000,
        tier=2,
        source="Calculated: Stipend (â‚¹120k/year) - Counterfactual informal wage (â‚¹168k/year)",
        unit="INR/year",
    ...
    APPRENTICE_INITIAL_PREMIUM: Parameter = field(default_factory=lambda: Parameter(
        ...
        description="Initial annual wage premium for apprentice. "
                    "NOTE: Back-of-envelope calculation gives ~₹235k/year, but we use "
                    "conservative ₹84k for modeling. Sensitivity range: [₹50k, ₹120k]."
    ))
...
class CounterfactualDistribution:
    ...
    in high-formal regions (e.g., South: 0.12 → 0.14) and understated in 
    low-formal regions (e.g., East: 0.12 → 0.10).
    """
...
class MincerWageModel:
    ...
    Core equation:
    W_t = exp(β₀ + β₁×Education + β₂×Experience + β₃×Experience²) × I(Formal) × FM
    
    Where:
    - β₁ = 0.058 (Mincer return)
    - β₂ = 0.00885 (experience linear)
    - β₃ = -0.000123 (experience quadratic)
    ...
    def generate_wage_trajectory(
        ...
        Returns:
            Array of annual wages (monthly × 12)
        """
...
class SectorTransitionModel:
    ...
    def get_expected_formal_years(
        ...
        For absorbing states: E[formal years] = p_formal × total_years
        """
        if self.absorbing:
...
class LifetimeNPVCalculator:
    ...
    def calculate_treatment_trajectory(
        ...
        if intervention == Intervention.RTE:
            ...
            # uplift over a notional annual baseline of 12 months × ₹20,000 = ₹240,000.
            # With registry value ₹84,000: 84,000 / 240,000 ≈ 0.35 ⇒ ~35% initial premium.
            #
            # NOTE: Back-of-envelope calculation in documentation gives ~₹235k/year
            # premium, but we intentionally use conservative ₹84k for modeling.
            # Sensitivity range [₹50k, ₹120k] is captured in parameter min/max values.
    ...
    def calculate_lnpv(
        ...
        - Treatment receives stipend (â‚¹120k/year)
        - Control earns informal wage (â‚¹168k/year) 
    ...
    def calculate_all_scenarios(self) -> List[Dict]:
        ...
        2 interventions × 4 regions × 4 demographics = 32 scenarios
        """
        results = []
...
def format_scenario_comparison(results: Dict[str, Dict]) -> str:
    ...
    output += "  * Moderate (40%): 2× national average (requires selection/urban effects)\n"
...
def adjust_npv_to_intervention_year(
    ...
    Args:
        ...
        years_to_entry: Years from intervention to labor market entry
                       - RTE: 16 years (enroll age 6 → entry age 22)
                       - Apprenticeship: 0-2 years (start age 18-20 → entry age 18-20)
...
def demonstrate_npv_conversion():
    ...
    years_rte = 16  # Age 6 enrollment → age 22 entry
    ...
    print(f"  → This is our standard output")
    print(f"  → In '2025 prices' (no inflation projection)")
    print(f"  → Base year = 2041 (when earnings start)")
    print(f"\nNPV at intervention year (2025): {format_currency(npv_at_intervention_rte)}")
    print(f"  → Further discounted by 16 years (delay before benefits)")
    print(f"  → Useful for comparing to 2025 program costs")
    print(f"  → Reduction: {(1 - npv_at_intervention_rte/npv_at_entry_rte)*100:.1f}%")
    ...
    print(f"  → Same value (years_to_entry = 0)")
    print(f"  → Apprenticeship benefits start immediately after training")
    print("="*70)
    print("\nKEY INSIGHT: RTE has longer delay → larger discount adjustment")
